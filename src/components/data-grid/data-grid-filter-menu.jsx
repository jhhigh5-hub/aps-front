...
function DataGridFilterItem(
  ...
  const onOperatorChange = React.useCallback((newOperator) => {
  ...
  const onEndValueChange = React.useCallback((newValue) => {
    onFilterUpdate(filter.id, {
      value: {
        operator,
        value: filterValue?.value,
                ...
                <CommandGroup>
                  {columns.map((column) => (
                    <CommandItem
                      ...
                      <Check
                        className={cn("ms-auto", column.id === filter.id ? "opacity-100" : "opacity-0")} />
                    </CommandItem>
                  ))}
                </CommandGroup>
              </CommandList>
...
function DataGridFilterInput(
  ...
  const debouncedOnEndValueChange = useDebouncedCallback((newValue) => {
  ...
  if (isSelectVariant && selectOptions.length > 0) {
    ...
    if (isMultiValueOperator) {
      ...
      const selectedOptionsWithIcons = selectedOptions.filter((selectedOption) => selectedOption.icon);
                ...
                <>
                  {selectedOptionsWithIcons.length > 0 && (
                    <div className="flex items-center -space-x-2 rtl:space-x-reverse">
                      {selectedOptionsWithIcons.map((selectedOption) =>
                        selectedOption.icon && (
                          <div
