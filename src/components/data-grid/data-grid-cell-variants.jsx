...
export function ShortTextCell(
  ...
  const onWrapperKeyDown = React.useCallback((event) => {
    if (isEditing) {
      if (event.key === "Enter") {
...
export function CheckboxCell(
  ...
  const onWrapperClick = React.useCallback((event) => {
...
export function MultiSelectCell(
  ...
  const removeValue = React.useCallback((valueToRemove, event) => {
    if (readOnly) return;
    event?.stopPropagation();
    event?.preventDefault();
      ...
      {isEditing ? (
        <Popover open={isEditing} onOpenChange={onOpenChange}>
          ...
          <PopoverContent
            ...
            onOpenAutoFocus={onOpenAutoFocus}>
            <Command
              className="**:data-[slot=command-input-wrapper]:h-auto **:data-[slot=command-input-wrapper]:border-none **:data-[slot=command-input-wrapper]:p-0 [&_[data-slot=command-input-wrapper]_svg]:hidden">
              <div
...
export function DateCell(
  ...
  const onDateSelect = React.useCallback((date) => {
...
export function FileCell(
  ...
  const addFiles = React.useCallback(async (newFiles, skipUpload = false) => {
  ...
  const removeFile = React.useCallback(async (fileId) => {
    if (readOnly || isPending) return;
    setError(null);
  ...
  const onCellDrop = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
    setIsDraggingOver(false);
  ...
  const onDropzoneDrop = React.useCallback((event) => {
    event.preventDefault();
    event.stopPropagation();
    setIsDragging(false);
  ...
  const onDropzoneKeyDown = React.useCallback((event) => {
