...
function useDataGrid(
  ...
  const prevCellSelectionMapRef = useLazyRef(() => new Map());
  ...
  const visualRowIndexCacheRef = React.useRef(null);
  ...
  const getVisualRowIndex = React.useCallback(rowId => {
  ...
  const onDataUpdate = React.useCallback((updates) => {
    ...
    const newData = new Array(tableRowCount);
    ...
    for (let i = 0; i < tableRowCount; i++) {
      ...
      if (updates) {
        const baseRow = existingRow ?? tableRow?.original ?? ({});
        const updatedRow = {
          ...baseRow
        };
        ...
        newData[i] = existingRow ?? tableRow?.original ?? ({});
      }
    }
  ...
  const getIsCellSelected = React.useCallback((rowIndex, columnId) => {
  ...
  const selectColumn = React.useCallback((columnId) => {
    const currentTable = tableRef.current;
    const rows = currentTable?.getRowModel().rows ?? [];
    const rowCount = rows.length ?? propsRef.current.data.length;
  ...
  const selectRange = React.useCallback((start, end, isSelecting = false) => {
  ...
  const onCellsCopy = React.useCallback(async () => {
    ...
    try {
      ...
      toast.success(`${selectedCellsArray.length} cell${
  ...
  const restoreFocus = React.useCallback((element) => {
  ...
  const onCellsPaste = React.useCallback(async (expandRows = false) => {
    if (propsRef.current.readOnly) return;
    ...
    try {
      ...
      if (updates.length > 0) {
        ...
        if (cellsSkipped > 0) {
          toast.success(`${cellsUpdated} cell${
            cellsUpdated !== 1 ? "s" : ""
  ...
  const focusCellWrapper = React.useCallback((rowIndex, columnId) => {
  ...
  const focusCell = React.useCallback((rowIndex, columnId) => {
    store.batch(() => {
  ...
  const onRowsDelete = React.useCallback(async (rowIndices) => {
    if (
  ...
  const navigateCell = React.useCallback((direction) => {
    const currentState = store.getState();
    if (!currentState.focusedCell) return;
  ...
  const onCellEditingStart = React.useCallback((rowIndex, columnId) => {
    if (propsRef.current.readOnly) return;
  ...
  const onCellEditingStop = React.useCallback((opts) => {
    const currentState = store.getState();
  ...
  const onSearchOpenChange = React.useCallback((open) => {
    if (open) {
  ...
  const onSearchQueryChange = React.useCallback((query) => store.setState("searchQuery", query), [store]);
  ...
  const getIsSearchMatch = React.useCallback((rowIndex, columnId) => {
  ...
  const getIsActiveSearchMatch = React.useCallback((rowIndex, columnId) => {
    const currentState = store.getState();
  ...
  const onCellClick = React.useCallback((rowIndex, columnId, event) => {
    if (event?.button === 2) {
      return;
    }
  ...
  const onCellDoubleClick = React.useCallback((rowIndex, columnId, event) => {
    if (event?.defaultPrevented) return;

    onCellEditingStart(rowIndex, columnId);
  }, [onCellEditingStart]);

  const onCellMouseDown = React.useCallback((rowIndex, columnId, event) => {
    if (event.button === 2) {
      return;
    }
  ...
  const onCellMouseEnter = React.useCallback((rowIndex, columnId) => {
    const currentState = store.getState();
    if (
  ...
  const onCellContextMenu = React.useCallback((rowIndex, columnId, event) => {
    event.preventDefault();
  ...
  const onContextMenuOpenChange = React.useCallback((open) => {
    if (!open) {
  ...
  const onRowSelectionChange = React.useCallback((updater) => {
    const currentState = store.getState();
  ...
  const onRowSelect = React.useCallback((rowIndex, selected, shiftKey) => {
    const currentState = store.getState();
    const rows = tableRef.current?.getRowModel().rows ?? [];
  ...
  const onRowHeightChange = React.useCallback((updater) => {
    const currentState = store.getState();
    const newRowHeight =
      typeof updater === "function"
  ...
  const onColumnClick = React.useCallback((columnId) => {
  ...
  const onPasteDialogOpenChange = React.useCallback((open) => {
    if (!open) {
      store.setState("pasteDialog", {
        open: false,
        rowsNeeded: 0,
  ...
  const isFirefox = React.useSyncExternalStore(React.useCallback(() => () => {}, []), React.useCallback(() => {
  ...
  const onScrollToRow = React.useCallback(async (opts) => {
    ...
    async function onScrollAndFocus(retryCount) {
  ...
  const onRowAdd = React.useCallback(async (event) => {
  ...
  const onDataGridKeyDown = React.useCallback((event) => {
    const currentState = store.getState();
  ...
  React.useEffect(() => {
    ...
    function onFocusOut(event) {
      ...
      const isFocusMovingOutsideGrid =
        !relatedTarget || !currentContainer.contains(relatedTarget);
